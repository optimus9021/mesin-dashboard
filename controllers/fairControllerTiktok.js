    ...
    try {
        const [rows] = await db.query(`
            SELECT 
                la.kategori, 
                u.username, 
                AVG(p.playCount) AS interaction
            FROM listAkun la
            JOIN users u ON la.username = u.username
...
module.exports = { getFollowersTiktok, getActivitiesTiktok, getInteractionTiktok, getResponsivenessTiktok, getLikesUserTiktok, getAllDataPostTiktok};
